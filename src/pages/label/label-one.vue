	...
	<div>
		<el-breadcrumb separator="/">
			<el-breadcrumb-item>行业标签管理</el-breadcrumb-item>
		  	<el-breadcrumb-item>一级标签列表</el-breadcrumb-item>
		</el-breadcrumb>
		<table class="tableStyle" style="margin-top:25px;">
			...
			<tbody>
				...
				<tr v-for="(i,index) in lists">
					<td><router-link :to="{name:'label-two',query:{id:i.id}}" class="link">{{i.name}}</router-link></td>
					<td>
						<router-link :to="{name:'label-two',query:{id:i.id}}" class="link">查看下级标签</router-link>
...
<script>
	...
	import {getFoundationList,vocationOneList} from '../../ajax/get.js'
	export default {
		data() {
	      	return {
	      		name:null,
	    ...
	    methods:{
	    	...
	    	getfundlist(){
		    	...
		    	vocationOneList().then((res) => {
					this.lists = res.vocationOneList
				}) 
		    }
	    },
	    mounted:function(){
